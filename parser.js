...
ArticleDoc.prototype.node_array_text = function(path) {
    var nodes = this.xp.select(path + '/text()', this.doc);
    var result = nodes.map(String);
    return result;
};

ArticleDoc.prototype.node_text = function(path) {
    var query = path + '//text()';
    var nodes = this.xp.select(query, this.doc);
    return nodes.join('');
};

ArticleDoc.prototype.node_raw = function(path) {
    reps = [{ from:'<bold>', to: '<b>'},
            { from:'</bold>', to: '</b>'},
            { from:'<italic>', to: '<em>'},
            { from:'</italic>', to: '</em>'}];
...
ArticleDoc.prototype.build_aff = function(node) {
...
ArticleDoc.prototype.add_affiliations = function(auth_node, author) {
...
ArticleDoc.prototype.add_mail = function(auth_node, author) {
...
ArticleDoc.prototype.author_array = function() {
    authors = [];
...
ArticleDoc.prototype.newArticle = function() {
